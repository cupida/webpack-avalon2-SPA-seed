 ...
 */(function (global, factory) {
    ...
    var valiDir = avalon.directive('validate', {
        ...
        validate: function validate(field, isValidateAll, event) {
            ...
            /* istanbul ignore if */
            if (elem.disabled || avalon.css(elem, 'display') === 'none' || avalon.css(elem, 'visibility') === 'hidden' || (elem.offsetWidth === 0 && elem.offsetHeight === 0)) return;
            var rules = field.vdom.rules;
            var ngs = [],
                isOk = true;
            if (!((rules.norequired || !rules.required) && value === '')) {
